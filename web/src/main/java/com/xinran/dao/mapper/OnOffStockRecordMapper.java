...
public interface OnOffStockRecordMapper {    
    ...
    @Insert("INSERT INTO on_off_stock_record(created_At,updated_At,book_Id,book_Status,owner_User_Id,owner_Phone,location,on_Stock_Date,off_Stock_Date,borrow_Status) "
    + " values(now(),now(),#{bookId},#{bookStatus},#{ownerUserId},#{ownerPhone},#{location},#{onStockDate},#{offStockDate},#{borrowStatus})")
